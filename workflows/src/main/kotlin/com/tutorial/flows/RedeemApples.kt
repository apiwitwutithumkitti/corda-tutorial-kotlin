...
class RedeemApplesInitiator (private val buyer : Party, private val stampId: UniqueIdentifier) : FlowLogic<SignedTransaction>() {
    ...
    override fun call(): SignedTransaction {
        ...
        */
        ...
        // final Party notary = getServiceHub().networkMapCache().getNotary(CordaX500Name.parse("O=Notary,L=London,C=GB")); // METHOD 2
        ...
        val appleStampStateAndRef: StateAndRef<*> = serviceHub
            .vaultService.queryBy(AppleStamp::class.java, inputCriteria).states[0]
        ...
        val BasketOfApplesStateAndRef: StateAndRef<*> = serviceHub
            .vaultService.queryBy(BasketOfApples::class.java, outputCriteria).states[0]
        ...
        val fullSignedTx = subFlow(CollectSignaturesFlow(partSignedTx, listOf(otherPartySession)))

        // Notaries and record the transaction in both parties' vaults.
        return subFlow(FinalityFlow(fullSignedTx, listOf(otherPartySession)))
    }
}

@InitiatedBy(RedeemApplesInitiator::class)
private class RedeemApplesResponder(private val couterpartySession: FlowSession) : FlowLogic<SignedTransaction>() {

    @Suspendable
    @Throws(FlowException::class)
    override fun call(): SignedTransaction {
        val signedTransaction = subFlow(object : SignTransactionFlow(couterpartySession){
            @Throws(FlowException::class)
        ...
        return subFlow(ReceiveFinalityFlow(couterpartySession, signedTransaction.id))
    }
